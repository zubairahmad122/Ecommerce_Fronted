...
const Navbar = () => {
  ...
  return (
    <header className='fixed top-0 left-0 w-full z-[100]'>
      ...
      {/* Announcement bar */}
      {STORE.announcement && annOpen && (
        <div className='bg-indigo-600 text-white text-[12px] font-medium text-center py-2 px-10 relative'>
          {STORE.announcement}
          <button onClick={() => setAnnOpen(false)} className='absolute right-3 top-1/2 -translate-y-1/2 opacity-70 hover:opacity-100'>
            <BiX size={16} />
          </button>
        </div>
      )}
      ...
      {/* Main nav */}
      <nav className={`transition-all duration-300 ${scrolled ? 'bg-white/95 backdrop-blur-xl shadow-md border-b border-slate-100' : 'bg-white border-b border-slate-100'}`}>
      ...
      {/* Mobile dropdown */}
      <div className={`md:hidden overflow-hidden transition-all duration-300 bg-white border-b border-slate-100 shadow-lg ${navOpen ? 'max-h-96' : 'max-h-0'}`}>
          ...
          <li className='pt-2 border-t border-slate-100 flex items-center gap-3'>
            {token ? (
              <>
                ...
                <button onClick={() => { logout(); close() }} className='ml-auto text-red-500 text-[13px] font-medium'>
                  Logout
                </button>
              </>
            ) : (
              <div className='flex gap-2 w-full'>
                <Link to='/login' onClick={close} className='flex-1 text-center px-4 py-2.5 border border-slate-200 text-slate-700 text-[14px] font-semibold rounded-lg'>
                  Sign In
                </Link>
                <Link to='/register' onClick={close} className='flex-1 text-center px-4 py-2.5 bg-indigo-600 text-white text-[14px] font-semibold rounded-lg'>
                  Register
                </Link>
              </div>
            )}
